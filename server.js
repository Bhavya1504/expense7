...
app.post('/expenses/add', (req, res) => {
  ...
  const insertExpenseQuery = 'INSERT INTO expenses (expenseName, price, category) VALUES (?, ?, ?)';
  connection.query(insertExpenseQuery, [expenseName, price, category], (error, result) => {
    if (error) {
      console.error('Error executing MySQL query: ' + error.stack);
...
app.delete('/expenses/delete/:id', (req, res) => {
  ...
  // Delete the expense from the database based on the expenseId
  const deleteExpenseQuery = 'DELETE FROM expenses WHERE id = ?';
  connection.query(deleteExpenseQuery, [expenseId], (error, result) => {
    if (error) {
      console.error('Error executing MySQL query: ' + error.stack);
